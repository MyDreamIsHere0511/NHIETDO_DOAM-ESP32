...
#include "modbus_tcp.h"     // Modbus TCP Server (Công nghiệp)n
...
void setup()
{
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C))
  {
    ...
    display.println("Modbus Port: 502"); // ← THÊM MỚI    display.println("Modbus Port: 502");  // ← THÊM MỚI    display.println("Modbus Port: 502");  // ← THÊM MỚI
  }
  else
  {
  ...
  if (saved_ssid.length() > 1)
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      display.println("Modbus Port: 502");
      display.display();
      delay(3000);

      setupNormalMode();
      setupModbusTCP(); // Khởi tạo Modbus TCP Server
    }
    else
    {
...
void loop()
{
  ...
  // Relay BẬT khi: Nhiệt độ > 32°C HOẶC đang ấn nút BOOT HOẶC điều khiển từ Modbus
  // Relay TẮT khi: Tất cả điều kiện trên đều FALSE
  if (temp > TEMP_THRESHOLD || btnState == LOW || modbusRelayControl)
  {
  ...
  // 4. Xử lý Modbus TCP (chỉ khi đã kết nối WiFi)
  if (!isConfigMode)
  {
    handleModbusTCP();
  }
